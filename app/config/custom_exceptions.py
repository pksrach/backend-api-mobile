...
def custom_validation_exception_handler(request: Request, exc: Any):
    return JSONResponse(
        status_code=422,
        content={"status": 422, "error": "Validation Error", "details": exc.errors()},
    )
...
def custom_authentication_exception_handler(request: Request, exc: Any):
    if exc.status_code == 401:
        return JSONResponse(
            status_code=401, content={"status": 401, "error": "Not authenticated"}
        )
    return JSONResponse(
        status_code=exc.status_code,
...
def attribute_error_exception(request: Request, exc: Any):
    return JSONResponse(
        status_code=500, content={"status": 500, "error": "Internal Server Error"}
    )
...
class ExceptionHandlerRegistry:
    ...
    def register_exception_handlers(app: FastAPI):
        ...
        app.add_exception_handler(
            HTTPException, custom_authentication_exception_handler
        )
